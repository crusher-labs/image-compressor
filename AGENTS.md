...
Single-purpose static tool, built as a **world page**: baggage drop. Compress an image in the browser on a self-service bag drop: the image is the bag on the belt, a seven-segment scale counts its size in KB down as the quality changes, an allowance in KB turns the lamp green or red (OVERWEIGHT), Fit to allowance searches for the best quality under the limit (stepping the longest side down if it must), and check-in downloads the result with a printed tag of size, dimensions, format and saving. JPEG, WebP or lossless PNG resize. Nothing uploaded. Part of the crusher-labs static tools line. Hosted on GitHub Pages at https://crusher-labs.github.io/image-compressor/
